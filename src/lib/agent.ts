...
export type SetUriPlan = {
  name: string;
  agentId: bigint;
  owner: Address;
  /** What the adapter/registry report today. */
  currentURI: string;
  desiredURI: string;
  alreadySet: boolean;
  info: AgentInfo;
...
/**
 * Plan `Adapter8004.setAgentURI(agentId, uri)` for an agent bound to `name`.
 * The adapter holds the agent NFT and forwards the update to the ERC-8004
 * IdentityRegistry only if `msg.sender` currently controls the bound token
 * (`_requireController`), so this refuses unless the binding is live and the
 * wallet is its controller — an orphaned binding (token id regenerated by a
 * role change) can never be repaired this way. No-op when the URI already
 * matches. The URI should dereference to an ERC-8004 `registration-v1` JSON.
 */
export async function setUriPlan(client: PublicClient, d: EnsV2Deployment, name: string, owner: Address, agentId: bigint, uri: string): Promise<SetUriPlan> {
  if (!uri || !/^(https?:\/\/|ipfs:\/\/)\S+$/i.test(uri)) {
    throw new ReadError("ENSV2_INVALID_AGENT_URI", `'${uri}' is not an http(s):// or ipfs:// URI.`, "ERC-8004 verifiers dereference agentURI expecting a registration JSON.");
  }
  const info = await agentInfo(client, d, name, agentId);
  if (info.orphaned || info.status !== "bound") {
    throw new ReadError("ENSV2_AGENT_NOT_BOUND", `Agent #${agentId} is ${info.status} for ${info.name}; the adapter will refuse setAgentURI.`, "Only a live binding can be updated. See `ensv2 agent info`.");
  }
  const controller = await client.readContract({ address: d.adapter8004, abi: adapter8004Abi, functionName: "isController", args: [agentId, owner] });
  if (!controller) {
    throw new ReadError("ENSV2_NOT_CONTROLLER", `${owner} does not control agent #${agentId} (the name's current owner does).`, "Run this from the wallet that owns the name.");
  }
  const alreadySet = info.agentURI === uri;
  const calldata = alreadySet ? null : { to: d.adapter8004, data: encodeFunctionData({ abi: adapter8004Abi, functionName: "setAgentURI", args: [agentId, uri] }), value: 0n };
  return { name: info.name, agentId, owner, currentURI: info.agentURI, desiredURI: uri, alreadySet, info, calldata };
}
